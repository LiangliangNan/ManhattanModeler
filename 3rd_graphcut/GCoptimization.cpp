...
OLGA_INLINE void GCoptimization::addterm2_checked(EnergyT* e, VarID i, VarID j, EnergyTermType e_00, EnergyTermType e_01, EnergyTermType e_10, EnergyTermType e_11, EnergyTermType w)
{
	if ( e_00 > GCO_MAX_ENERGYTERM || e_11 > GCO_MAX_ENERGYTERM || e_01 > GCO_MAX_ENERGYTERM || e_10 > GCO_MAX_ENERGYTERM )
		handleError("Smooth cost term was larger than GCO_MAX_ENERGYTERM; danger of integer overflow.");
	if ( w > GCO_MAX_ENERGYTERM )
		handleError("Smoothness weight was larger than GCO_MAX_ENERGYTERM; danger of integer overflow.");
	...
	if ( e_00+e_11 > e_01+e_10 )
		handleError("Non-submodular expansion term detected; smooth costs must be a metric for expansion");
	m_beforeExpansionEnergy += e_11*w;
	e->add_term2(i,j,e_00*w,e_01*w,e_10*w,e_11*w);
}
